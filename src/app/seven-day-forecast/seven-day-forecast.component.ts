...
export class SevenDayForecastComponent implements OnInit {
  ...
  async switchLocations(zip: number){
    this.currentForecast = <Object[]> await this.forecastService.getForecast(zip.toString());
    this.currentUVForecast = <Object[]> await this.forecastService.getUVForecast(zip.toString());
    for (let i = 0; i < 40; i++){
      this.forcastTemps[i] = this.currentForecast.list[i].main.temp;
    }
    for (let i = 0; i < 40; i++){
